...
def get_sequential_features(
    batch: Dict[str, torch.Tensor], device: torch.device, max_output_length: int
    ...
    historial_keys_map = {
        ...
        "historical_item_click_times": "timestamps",  # For relative time attention bias in HSTU
    ...
    if max_output_length > 0:
        ...
        for key, new_key in historial_keys_map.items():
            ...
            payloads[new_key] = torch.concat([payload, padding_payload], dim=1)  # [B, N + max_output_length, ...]
    else:
        for key, new_key in historial_keys_map.items():
    ...
    seq_features = SequentialFeatures(past_lens=past_lens, past_ids=past_ids, past_payloads=payloads)
    del batch
